...
export class ChatGateway {
  ...
  async handleAiChatMessage(
    ...
    if (messages.length === 0) {
      const newUserMessage = await this.createMessageService.execute({ role: MessagesRole.user, content, chat_id, user_id });

      socket.emit('ai-chat-message', JSON.stringify({
      ...
      if (response) {
        const newAiMessage = await this.createMessageService.execute({
      ...
      const newUserMessage = await this.createMessageService.execute({
        role: MessagesRole.user,
        content,
        chat_id,
        user_id
